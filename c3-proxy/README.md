...
The OreSat C3 Proxy Board is a debugging tool for OreSat cards. It provides a proxy for the C3 card and external power, which is all you need to work on an OreSat card. In particular, it provides:

* An MCP2221 IC which connects to a PC with USB 2.0 and provides I2C, UART, and GPIO. The I2C is used to control the OreSat Power Domain (OPD) which turns the card on and off. The UART is used as a `C3-UART` proxy, providing a proxy for the C3's backplane UART channel. Finally the GPIO is used to assert !SHUTDOWN.
* An INA260 I2C current sense IC which allows the C3 proxy board to measure the power consumption of the card with  a few hundred Hz of bandwidth and down to the 100s of microamps.
* An external barrel jack for providing  external power from a wall mounted AC to DC converter

A few warnings:

* DO NOT USE THIS BOARD FOR HIGH CURRENT DEVICES, LIKE REACTION WHEELS OR ANTENNA DEPLOYERS. YOU'LL BLOW THE EXT POWER SUPPLY FUSE.
* CAREFUL WITH THE `C3-UART` SWITCH, BE SURE IT'S USUALLY IN THE "DISCONNECTED" STATE SO IT DOESN'T FIGHT WITH AN EXISTING C3 CARD.
...
- 4 layer board
- Bounding box is 74.75 x 50.00 mm
...
- Slots
  - There are 7 plated slots slots.
- Cutouts
...
- Board Surface treatment should be ENIG, although immersion Silver is acceptable.
